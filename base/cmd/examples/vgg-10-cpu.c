...
static int globErrFn(const char *epath, int eerrno) {
...
static dataset getDataset(Context *ctx) {
  glob_t batchFiles;
  ...
  Array_Tensor Xtrain = shapes_Make_DynamicTensorArray(ctx->memory);
  Array_Tensor Ytrain = shapes_Make_DynamicTensorArray(ctx->memory);
  Array_Tensor Xtest = shapes_Make_DynamicTensorArray(ctx->memory);
  Array_Tensor Ytest = shapes_Make_DynamicTensorArray(ctx->memory);
...
static inline ArrayPair toBatches(Context *ctx, Array *X, Array *Y) {
  ...
  Array *Xbatched = shapes_Make_DynamicTensorArray(ctx->memory);
  Array *Ybatched = shapes_Make_DynamicTensorArray(ctx->memory);
...
static FowardPassOp *Make_ConvBlock(Context *ctx) {
...
static FowardPassOp *Make_LinearBlock(Context *ctx, u8 numLabels) {
...
static FowardPassOp *Make_Model(Context *ctx, u8 numLabels) {
...
static FowardPassOp *runTraining(Context *hostCtx, dataset ds) {
  u8 numLabels = ds.labels->size;
  FowardPassOp *model = Make_Model(hostCtx, numLabels);
  Optimizer *optimzer = shapesnn_SGD(hostCtx, LEARNING_RATE);
  ...
  Context scratch = shapes_GetScratchContext(hostCtx, 4 * GB);
...
static void runInference(Context *hostCtx, FowardPassOp *model, dataset ds) {
  ArrayPair testDs = toBatches(hostCtx, ds.Xtest, ds.Ytest);
  Array *Xtest = testDs.a;
  Array *Ytest = testDs.b;

  Context scratch = shapes_GetScratchContext(hostCtx, 5 * GB);
