...
template <typename T>
static Result launchAddKernel(const void *a, const void *b, void *dest,
                              size_t n) {
...
template <typename T>
static Result launchSubtractKernel(const void *a, const void *b, void *dest,
                                   size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchMultiplyKernel(const void *a, const void *b, void *dest,
                                   size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchGreaterKernel(const void *a, const void *b, void *dest,
                                  size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchGreaterOrEqualKernel(const void *a, const void *b,
                                         void *dest, size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchLessKernel(const void *a, const void *b, void *dest,
                               size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchLessOrEqualKernel(const void *a, const void *b, void *dest,
                                      size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchEqualKernel(const void *a, const void *b, void *dest,
                                size_t n) {
  int threadsPerBlock = 256;
  int blocks =
      (int)((n + (size_t)threadsPerBlock - 1) / (size_t)threadsPerBlock);
...
template <typename T>
static Result launchBroadcastAddKernel(const void *larger, const void *smaller,
...
template <typename T>
static Result
launchBroadcastSubtractKernel(const void *larger, const void *smaller,
                              void *dest, size_t outerDimSize,
                              size_t broadcastDimSize, size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
...
template <typename T>
static Result
launchBroadcastMultiplyKernel(const void *larger, const void *smaller,
                              void *dest, size_t outerDimSize,
                              size_t broadcastDimSize, size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
...
template <typename T>
static Result
launchBroadcastGreaterKernel(const void *larger, const void *smaller,
                             void *dest, size_t outerDimSize,
                             size_t broadcastDimSize, size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
...
static Result launchBroadcastGreaterOrEqualKernel(
    const void *larger, const void *smaller, void *dest, size_t outerDimSize,
    size_t broadcastDimSize, size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
            (innerDimSize + block.x - 1) / block.x);
...
template <typename T>
static Result launchBroadcastLessKernel(const void *larger, const void *smaller,
                                        void *dest, size_t outerDimSize,
                                        size_t broadcastDimSize,
                                        size_t innerDimSize) {
  dim3 block(32, 32, 1);
...
template <typename T>
static Result
launchBroadcastLessOrEqualKernel(const void *larger, const void *smaller,
                                 void *dest, size_t outerDimSize,
                                 size_t broadcastDimSize, size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
...
template <typename T>
static Result
launchBroadcastEqualKernel(const void *larger, const void *smaller, void *dest,
                           size_t outerDimSize, size_t broadcastDimSize,
                           size_t innerDimSize) {
  dim3 block(32, 32, 1);
  dim3 grid(outerDimSize, (broadcastDimSize + block.y - 1) / block.y,
            (innerDimSize + block.x - 1) / block.x);
...
runCudaBroadcastBinaryOp(shapes_Dtype dtype, shapes_OpType opType, void *larger,
  ...
  return fn(larger, smaller, dest, outerDimSize, broadcastDimSize,
            innerDimSize);
}
