...
func indexWithTensorBackward(ctx *Context, node *ComputationGraphNode) {
	...
	res := C.wrap_IndexAccumulate1d(cCtx, x.Grad().cTensor, indices.cTensor, node.Grad.cTensor)
	if res != C.OK {
...
func indexWithTensor2dBackward(ctx *Context, node *ComputationGraphNode) {
	...
	cCtx := (*C.Context)(ctx.UnsafePtr())
	res := C.wrap_IndexAccumulate2d(cCtx, x.Grad().cTensor, rowIndices.cTensor, colIndices.cTensor,
		node.Grad.cTensor)
	if res != C.OK {
