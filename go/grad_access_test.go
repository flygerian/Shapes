...
// x shape [3, 4], y = x.Get(ctx, 1), loss = sum(y), grad should be 1 at row 1, 0 elsewhere.
func TestGetTensorAtBackward(t *testing.T) {
	ctx := New(context.Background(), WithGrad(true))
	defer ctx.Finish()

	x := Float(ctx, Shape{3, 4}, 0.0)
...
func TestSliceBackward(t *testing.T) {
	...
	x := Float(ctx, Shape{4, 4}, 0.0)

	// s = x[1:3, 1:3] -> 2x2 view
	s := x.Slice(ctx, Range{1, 3}, Range{1, 3})

	// Backward from scalar sum of s
	loss := s.Sum(ctx, 0).Sum(ctx, 1)
	loss.Backward(ctx)

	for i := range uint32(4) {
		for j := range uint32(4) {
			g := x.Grad().Get(ctx, i, j).Item().(float32)
...
// TestNestedViewBackward verifies backward through x[1:4, :].Get(ctx, 0) i.e. Slice then GetTensorAt.
func TestNestedViewBackward(t *testing.T) {
	ctx := New(context.Background(), WithGrad(true))
	defer ctx.Finish()

	// x: 4x3
	x := Float(ctx, Shape{4, 3}, 0.0)

	// s = x[1:4, :] -> 3x3 view
	s := x.Slice(ctx, Range{1, 4}, Range{0, 3})
...
func TestIndexWithTensorBackward(t *testing.T) {
	...
	indices := FromInt8(ctx, []int8{0, 2, 0})
	...
	for j := range uint32(3) {
		g := x.Grad().Get(ctx, 0, j).Item().(float32)
	...
	// x.grad[1, :] = 0.0
	for j := range uint32(3) {
		g := x.Grad().Get(ctx, 1, j).Item().(float32)
		if !approxEq(g, 0.0, 1e-5) {
	...
	// x.grad[2, :] = 1.0
	for j := range uint32(3) {
		g := x.Grad().Get(ctx, 2, j).Item().(float32)
		if !approxEq(g, 1.0, 1e-5) {
	...
	// x.grad[3, :] = 0.0
	for j := range uint32(3) {
		g := x.Grad().Get(ctx, 3, j).Item().(float32)
		if !approxEq(g, 0.0, 1e-5) {
...
func TestIndexWithTensor2dBackward(t *testing.T) {
	...
	rowIdx := FromInt8(ctx, []int8{0, 1, 0})
	colIdx := FromInt8(ctx, []int8{2, 3, 2})

	// y = x[rowIdx, colIdx] -> shape [3]
	y := x.Get(ctx, rowIdx, colIdx)
	...
	g02 := x.Grad().Get(ctx, 0, 2).Item().(float32)
	if !approxEq(g02, 2.0, 1e-5) {
		t.Errorf("x.grad[0,2] = %f, want 2.0", g02)
	}

	// x.grad[1,3] = 1.0
	g13 := x.Grad().Get(ctx, 1, 3).Item().(float32)
