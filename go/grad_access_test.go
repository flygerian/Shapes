...
func TestGetTensorAtBackward(t *testing.T) {
	...
	for j := range uint32(4) {
		g0 := x.Grad().(*Tensor).Get(ctx, 0, j).Item().(float32)
		if !approxEq(g0, 0.0, 1e-5) {
			t.Errorf("x.grad[0,%d] = %f, want 0.0", j, g0)
		}
		g1 := x.Grad().(*Tensor).Get(ctx, 1, j).Item().(float32)
		if !approxEq(g1, 1.0, 1e-5) {
			t.Errorf("x.grad[1,%d] = %f, want 1.0", j, g1)
		}
		g2 := x.Grad().(*Tensor).Get(ctx, 2, j).Item().(float32)
...
func TestSliceBackward(t *testing.T) {
	...
	for i := range uint32(4) {
		for j := range uint32(4) {
			g := x.Grad().(*Tensor).Get(ctx, i, j).Item().(float32)
...
func TestIndexWithTensorBackward(t *testing.T) {
	...
	for j := range uint32(3) {
		g := x.Grad().(*Tensor).Get(ctx, 0, j).Item().(float32)
	...
	// x.grad[1, :] = 0.0
	for j := range uint32(3) {
		g := x.Grad().(*Tensor).Get(ctx, 1, j).Item().(float32)
		if !approxEq(g, 0.0, 1e-5) {
	...
	// x.grad[2, :] = 1.0
	for j := range uint32(3) {
		g := x.Grad().(*Tensor).Get(ctx, 2, j).Item().(float32)
		if !approxEq(g, 1.0, 1e-5) {
	...
	// x.grad[3, :] = 0.0
	for j := range uint32(3) {
		g := x.Grad().(*Tensor).Get(ctx, 3, j).Item().(float32)
		if !approxEq(g, 0.0, 1e-5) {
...
func TestIndexWithTensor2dBackward(t *testing.T) {
	...
	g02 := x.Grad().(*Tensor).Get(ctx, 0, 2).Item().(float32)
	if !approxEq(g02, 2.0, 1e-5) {
		t.Errorf("x.grad[0,2] = %f, want 2.0", g02)
	}

	// x.grad[1,3] = 1.0
	g13 := x.Grad().(*Tensor).Get(ctx, 1, 3).Item().(float32)
